...
from person
INNER join Madam_descrip on person.license_id =  Madam_descrip.id
inner join Madam_event on person.id= Madam_event.person_id
;
...
select facebook_event_checkin.person_id as person_iid, facebook_event_checkin.date, event_name,count(person_id) as no_of, name, person.id, drivers_license.id
from facebook_event_checkin
